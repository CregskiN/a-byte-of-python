...
你已经了解了，在你的程序中如何通过一次定义函数，多次重用代码。那如果，你想重用你写过的其他程序中的一些函数呢？你可能猜到了，答案就是模块。
...
有多种编写模块的方式，但最简单的就是创建一个以 `.py` 为扩展名的文件，里面包含函数和变量。
...
另一个方法是，用编写出 Python 解释器的语言编写模块。例如，你可以用 [C/C++](http://docs.python.org/3/extending/) 编写模块并编译，这样的模块可以在标准版 Python 解释器中被 Python 程序使用。
...
一个模块可以被*导入*到另一个程序中，以使用这个模块中的功能。这也是为什么，我们可以使用 Python 标准库。首先，我们会了解如何使用 Python 标准库模块。
...
首先，我们使用 `import` 语句导入了 `sys` 模块。基本上，这段代码就是向 Python 说明我们想要使用这个模块。`sys` 模块包含很多功能，这些功能关联于与 Python 解释器及其环境（即操作系统）。
...
当 Python 执行 `import sys` 语句，它会寻找 `sys` 模块。在这个例子中，`sys` 是内置模块之一，也因此 Python 直到去哪里找到它。
...
如果是一个未编译的模块（例如用 Python 编写的模块），那么 Python 解释器根据 `sys.path` 变量中罗列的路径，寻找这个模块。如果模块被找到了，模块内的语句会被执行，并且模块对你*开放使用*。注意，初始化只在第一次导入模块时才会进行。
...
`sys` 模块中的 `argv` 变量支持点方式访问（即 `sys.argv`）。这清晰地表明，`argv` 是 `sys` 模块的一部分。这种规则的另一个优点是，这不会与你程序中其他名为 `argv` 的变量有命名冲突。
...
变量 `sys.argv` 是一个字符串*列表*（有关列表的细节会在[之后的章节](https://python.swaroopch.com/data_structures.html#data-structures)说明）。具体来说，`sys.argv` 包含命令行参数的列表（即使用命令行时，传递给程序的参数）。

The `sys.argv` variable is a *list* of strings (lists are explained in detail in a [later chapter](https://python.swaroopch.com/data_structures.html#data-structures)). Specifically, the `sys.argv` contains the list of *command line arguments* i.e. the arguments passed to your program using the command line.

如果你使用 IDE 编写并运行程序，在菜单中找一下指定命令行参数的方式吧。
...
Importing a module is a relatively costly affair, so Python does some tricks to make it faster. One way is to create *byte-compiled* files with the extension `.pyc` which is an intermediate form that Python transforms the program into (remember the [introduction section](https://python.swaroopch.com/about_python.html#interpreted) on how Python works?). This `.pyc` file is useful when you import the module the next time from a different program - it will be much faster since a portion of the processing required in importing a module is already done. Also, these byte-compiled files are platform-independent.
...
Packages are just a convenience to organize modules hierarchically. You will see many instances of this in the [standard library](https://python.swaroopch.com/stdlib.html#stdlib).
